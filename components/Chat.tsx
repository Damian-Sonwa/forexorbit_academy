...
export default function Chat({ lessonId }: ChatProps) {
  ...
  const { socket, connected, joinLesson, leaveLesson, sendChatMessage, onChatMessage } = useSocket();
  ...
  return (
    <div className="flex flex-col h-[400px] sm:h-96 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-4 bg-gray-50 min-h-0">
        {messages.length === 0 ? (
      ...
      {/* Input */}
      <form onSubmit={handleSend} className="border-t border-gray-200 p-2 sm:p-4 bg-white flex-shrink-0">
        <div className="flex space-x-2 sm:space-x-3">
          <input
            ...
            disabled={!connected}
            className="flex-1 px-3 py-2 sm:px-4 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent disabled:bg-gray-100 disabled:cursor-not-allowed"
          />
          <button
            type="submit"
            disabled={!connected || !input.trim()}
            className="px-3 py-2 sm:px-6 sm:py-3 bg-primary-600 hover:bg-primary-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white rounded-xl font-semibold transition-colors shadow-sm flex-shrink-0"
          >
